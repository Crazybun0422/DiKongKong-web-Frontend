...
export const MAINLAND_PROVINCE_CODES = [
...
export function findNearestProvinceLayerRecord(records, bbox) {
  const rect = normalizeRectBbox(bbox)
  if (!rect) return null
...
export function buildProvinceLayerParams(records, bbox) {
  const matched = findIntersectingProvinceLayerRecords(records, bbox)
  // Some offshore tiles fall outside the simplified province polygon set. In that case,
  // fall back to the nearest province layer instead of returning an empty WMS request.
  const resolvedRecords = matched.length ? matched : [findNearestProvinceLayerRecord(records, bbox)].filter(Boolean)
  return {
