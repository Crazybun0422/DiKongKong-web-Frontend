...
const CATALOG_PATH = '/offline-layer-tiles/catalog'
...
const resolveOfflineTileAssetBase = () => {
  const configured = `${import.meta.env.VITE_UOM_TILE_ASSET_BASE || ''}`.trim()
  if (configured) return configured.replace(/\/+$/, '')
  return 'https://www.skylane.cn'
}

const normalizeRenderColorToVariant = (value) => {
  const normalized = `${value || ''}`.trim().toUpperCase()
  if (!normalized || normalized === 'DEFAULT' || normalized === '#01F4F4') return 'current'
  if (normalized === '#34C759') return 'green'
  if (normalized === '#FFB800') return 'gold'
  if (normalized === '#FF5A5F') return 'coral'
  return 'current'
}

const normalizeTileSize = (value, fallback = DEFAULT_TILE_SIZE) => {
...
const resolveViewportTileShape = (viewportWidth, viewportHeight, options = {}) => {
...
const normalizeSquareSide = (baseSide, maxTiles, maxIndex) => {
  ...
  if (Number.isFinite(maxTiles) && maxTiles > 0) {
    ...
    if (limited > 0) side = Math.min(side, limited)
  }
...
const normalizeRectangularTileCounts = (rawTilesX, rawTilesY, maxTiles, maxIndex) => {
  ...
  if (Number.isFinite(maxTiles) && maxTiles > 0) {
    ...
    while (tilesX * tilesY > maxTiles && (tilesX > 1 || tilesY > 1)) {
      if (tilesX >= tilesY && tilesX > 1) tilesX -= 1
      else if (tilesY > 1) tilesY -= 1
      else break
    }
  }
...
const normalizeAxisSpan = (value, maxIndex) => {
...
const lonLatToTileFloat = (lng, lat, zoom) => {
...
const tilePriorityDistance = (tileX, tileY, centerTileFloat) => {
  if (!centerTileFloat) return Number.POSITIVE_INFINITY
  const minDx =
    centerTileFloat.x < tileX
      ? tileX - centerTileFloat.x
      : centerTileFloat.x > tileX + 1
        ? centerTileFloat.x - (tileX + 1)
        : 0
  const minDy =
    centerTileFloat.y < tileY
      ? tileY - centerTileFloat.y
      : centerTileFloat.y > tileY + 1
        ? centerTileFloat.y - (tileY + 1)
        : 0
  return minDx * minDx + minDy * minDy
}

const buildCenteredAxisRange = (centerIndex, centerFloat, side, maxIndex) => {
...
const normalizeTileUrlTemplate = (value) => {
  const template = `${value || ''}`.trim()
  if (!template) return ''
  const assetBase = resolveOfflineTileAssetBase()
  if (/^https?:\/\//i.test(template)) {
    if (/^https?:\/\/[^/]+\/layer\//i.test(template)) {
      return template.replace(/^https?:\/\/[^/]+(?=\/layer\/)/i, assetBase)
    }
    return template.replace(/^https?:\/\/cdn\.skylane\.cn(?=\/|$)/i, assetBase)
  }
  if (template.startsWith('/')) return `${assetBase}${template}`
  return `${assetBase}/${template.replace(/^\/+/, '')}`
}

const fillUrlTemplate = (urlTemplate, zoom, tileX, tileY) => {
...
const normalizeTileCoordSystem = (value) => {
  const normalized = `${value || ''}`.trim().toLowerCase()
  if (!normalized) return 'gcj02'
  if (normalized === 'wgs' || normalized === 'wgs84' || normalized === 'epsg:4326') return 'wgs84'
  if (normalized === 'cgcs2000' || normalized === 'epsg:4490') return 'cgcs2000'
  return 'gcj02'
}

const convertDisplayCenterToTileCoord = (center, tileCoordSystem) => {
...
const convertTileCoordToDisplayPoint = (point, tileCoordSystem) => {
...
const normalizeCatalogPayload = (response) => {
  const payload = response?.data
  return payload?.data || payload || null
}

const resolveLayerTileCatalog = (options = {}) => {
  if (catalogCache && options.forceRefresh !== true) return Promise.resolve(catalogCache)
  if (catalogPromise && options.forceRefresh !== true) return catalogPromise
  catalogPromise = http.get(CATALOG_PATH).then((response) => {
    const catalog = normalizeCatalogPayload(response)
    if (!catalog || !Array.isArray(catalog.layers)) {
      throw new Error('offline-layer-catalog-missing')
    }
...
const resolveCatalogLayer = (options = {}) => {
  ...
  return catalog.layers.find((item) => `${item?.layerKey || ''}`.trim() === preferredLayerKey) || catalog.layers[0] || null
}

const resolveVariantEntry = (renderColor, options = {}) => {
...
const getCachedLayerTileDescriptor = (renderColor, options = {}) => {
  const catalog = options.catalog || catalogCache
  const layer = resolveCatalogLayer({ ...options, catalog })
  if (!layer) return null
  const variant = resolveVariantEntry(renderColor, { ...options, catalog, layer })
...
const resolveLayerTileDescriptor = (renderColor, options = {}) => {
  const cached = getCachedLayerTileDescriptor(renderColor, options)
  if (cached && options.forceRefresh !== true) return Promise.resolve(cached)
  return resolveLayerTileCatalog(options).then((catalog) => {
    const descriptor = getCachedLayerTileDescriptor(renderColor, { ...options, catalog })
    if (!descriptor) throw new Error('offline-layer-descriptor-missing')
    return descriptor
  })
}

const buildOfflineLayerTiles = (center, zoom, region, options = {}) => {
  ...
  if (!center || zoom < minZoom || zoom > maxZoom) return []
  ...
  if (viewportShape === 'square') {
    const squareSide = normalizeSquareSide(Math.max(rawTilesX, rawTilesY), Number(options.maxTiles), maxIndex)
    tilesX = squareSide
    tilesY = squareSide
  } else {
    const rect = normalizeRectangularTileCounts(rawTilesX, rawTilesY, Number(options.maxTiles), maxIndex)
    tilesX = rect.tilesX
    tilesY = rect.tilesY
  }
  ...
  if (tileCoords.length > 1) {
    tileCoords.sort((a, b) => {
      ...
      const dxA = Math.abs(a.x + 0.5 - centerTileFloat.x)
      const dyA = Math.abs(a.y + 0.5 - centerTileFloat.y)
      const dxB = Math.abs(b.x + 0.5 - centerTileFloat.x)
      const dyB = Math.abs(b.y + 0.5 - centerTileFloat.y)
      if (Math.abs(dxA - dxB) > 1e-9) return dxA - dxB
      if (Math.abs(dyA - dyB) > 1e-9) return dyA - dyB
